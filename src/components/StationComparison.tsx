...
export const StationComparison = ({ stations, onClear }: StationComparisonProps) => {
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">
          <div className="text-center">
            ...
            <div>{t.compare.emptyHint}</div>
            <div className="text-xs text-gray-400 mt-1">{t.compare.emptySubHint}</div>
          </div>
        </div>
      </div>
    );
  ...
  const option = {
    title: {
      text: `${t.compare.trendTitle}(${stations.length} ${t.compare.stations})`,
      left: 'center',
      top: 10,
    ...
    series: stations.map((station, idx) => ({
      ...
      itemStyle: { color: stationColors[idx % stationColors.length] },
      emphasis: {
        focus: 'series',
        lineStyle: { width: 4 },
      },
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-4 h-full flex flex-col">
      ...
      <div className="text-xs text-gray-500 mb-2">
        {t.compare.selectedCount.replace('{count}', String(stations.length))} · {t.compare.legendHint}
      </div>
